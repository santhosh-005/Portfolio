...
function Game() {
  ...
  useEffect(() => {
    ...
    return () => {
      clearInterval(intervalId1), clearInterval(intervalId2);
    };
  }, [playModal, time]);
  ...
  useEffect(() => {
    window.onkeyup = (e) => {
      if (!playModal && !playAgainModal && e.key == "Enter") {
        setBoyX((prev) => prev + 20);
      } else {
        setBotX();
      }
    };
  }, [playModal, playAgainModal]);
      ...
      <div className="gameBoard">
        ...
        {playAgainModal && (
          <div className="gameEndModal">
            ...
            />
            <h5>
              Bot trigger speed <span style={{ color: "red" }}>{botSpeed}</span>{" "}
              p/s
            </h5>
            <p>
              If you won at max speed, sent me screenshot and win suprise gift
            </p>
          </div>
        )}
        <hr />
        <img
          style={gameCharStyles}
          ref={useref}
          src={gameChar}
          id="gameChar"
          alt="game char"
